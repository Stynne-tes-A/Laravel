...
class="menu p-3   w-20  h-full grid bg-white border-r text-base-content" :class="{'w-72 ':!shrink}">
    ...
    <ul class="space-y-4 mt-2">
        ...
        <li>
            <a wire:navigate href="{{route('profile.home',auth()->user()->username)}}" class="flex items-center gap-5">
                ...
                <x-avatar src="{{auth()->user()->avatar}}" class=" w-7 h-7 shrink-0" />
    ...
    <button type="submit" class="bg-red-500 text-white py-2 px-4 rounded-md hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
        Logout
    </button>
</form>
